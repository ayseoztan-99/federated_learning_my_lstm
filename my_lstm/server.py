...
class AggregateMetricsStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        """Değerlendirme sonuçlarını topla ve kaydet"""
        if not results:
        ...
        # Her istemcinin sonuçlarını kaydet
        for client_id, result in results:
            self.metrics_records.append({
                "round": server_round,
                "client_id": result.metrics.get("client_id", client_id),
                "loss": result.loss,
        ...
        metrics_filename = f"metrics_epoch_{LOCAL_EPOCHS}_rounds_{NUM_ROUNDS}.csv"
        
        # Metrikleri CSV olarak kaydet
        df = pd.DataFrame(self.metrics_records)
        df.to_csv(os.path.join(RESULTS_DIR, metrics_filename), index=False)
...
def fit_config(server_round):
    """Eğitim yapılandırmasını döndür"""
    return {
        "epoch": LOCAL_EPOCHS,
        "learning_rate": 0.001  # İstemciler için öğrenme oranı
    }
...
if __name__ == "__main__":
    print(f"[Server] Starting server at {SERVER_ADDRESS}")
    print(f"[Server] Number of rounds: {NUM_ROUNDS}")
    print(f"[Server] Local epochs per round: {LOCAL_EPOCHS}")
